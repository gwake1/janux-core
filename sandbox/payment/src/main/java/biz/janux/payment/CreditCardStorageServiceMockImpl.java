...
public class CreditCardStorageServiceMockImpl implements CreditCardStorageService<SearchCriteria>{
		...
		if (((CreditCardImpl)creditCard).getId()==((CreditCardImpl)creditCard).UNSAVED_ID)
		{
			if (creditCard.isSwiped())
				if (creditCard.getTrack2()==null)
					throw new RuntimeException("the cc was swiped but the track2 is null");
				else
					token = getEncryptedStorageService().encryption(creditCard.getTrack2());
			else
	...
	public boolean isValidTrack2(String track2, String number) {
		return this.creditCardValidator.isValidTrack2(track2, number);
	}
